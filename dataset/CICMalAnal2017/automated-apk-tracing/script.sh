...
SOURCE_DIRECTORY=$(grep -oP '(?<=SOURCE_DIRECTORY=).*' config.txt )
echo -n "Source directory apks : "
echoColor b $SOURCE_DIRECTORY
...
prepareData()
{
    ...
    for FILE in $(find $SOURCE_DIRECTORY -name '*.apk');
    do
        [ -f "$FILE" ] || break
        #we start by hashing the file
        HASH=$(sha256sum $FILE | grep -oh "^[0-9A-Za-z]*")
        echoColor g "Moving $HASH"
        ...
        if [ -z "$TMPSTR" ]
        then
            TMPSTR=$(echo $LOCALDIR | grep "Benign" )
            if [ ! -z $TMPSTR ]
            then
...
extractAPKDataIntoDB()
{
    ...
    echo $buffering "" >> $DESTINATION_DIRECTORY/packageInfo.csv
...
processData()
{
    ...
    do
		...
		echo "l. 258"
            export -f createContainer traceAPK traceAPKMonkey traceAPKDroidbot moveOutput killContainer createContainerAndTrace echoColor
            echo "l. 260"
